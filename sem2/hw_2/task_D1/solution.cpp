...
bool expand (GameField &field, string &path, int depth, double coefficient, char prev = '0') {
	int heuristic = int(coefficient * field.heuristic());
	...
	if (heuristic == 0)
		...
		for (int i = 0; i < 4; i ++) {
			...
			if (expand(field, path, depth, coefficient, antiDirections[i]))
				return true;
...
bool IDAStar (const GameField start, string &path, double coefficient = 1.00f) {
	if (!start.isSolvable())
		return false;
	...
	for (int iteration = start.heuristic(); iteration <= start.godNumber; iteration += 2) {
		...
		if (expand(field, path, iteration, coefficient))
			return true;
	}
...
int main () {
	...
	if (start.heuristic() < 34)
		IDAStar(start, path);

	else if (start.heuristic() < 40)
		IDAStar(start, path, 1.05f);

	else
		IDAStar(start, path, 1.03f);
